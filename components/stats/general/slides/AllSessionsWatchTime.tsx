...
import { sessionsByWatchTime, totals } from '../data/statsData';

export default function AllSessionsWatchTime() {
  return (
    <SlideCard compact>
      {/* Header */}
      <div className="mb-6">
      ...
      <div className="max-h-[420px] overflow-y-auto pr-2 scrollbar-thin scrollbar-track-off-white scrollbar-thumb-neutral-3">
        <BarChart data={sessionsByWatchTime} compact />
      </div>
      ...
      <div className="mt-6 pt-5 border-t border-neutral-2 flex flex-col sm:flex-row justify-between items-center gap-2">
        ...
        <p className="text-sm font-extrabold text-brand-navy">
          Total Watch Time: <span className="text-primary-blue">{totals.totalWatchTime.toLocaleString()} hrs</span>
        </p>
      </div>
    </SlideCard>
  );
