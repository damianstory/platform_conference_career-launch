...
export default function ResourceCards({ resources, colSpan = 'lg:col-span-6', layout = 'grid' }: ResourceCardsProps) {
  ...
  // Limit resources based on layout: 3 for vertical, 6 for grid
      ...
      <div className="px-4 pt-4 pb-3">
        <div className={layout === 'vertical' ? 'flex flex-col gap-1' : 'grid grid-cols-1 sm:grid-cols-2 gap-1'}>
          {displayResources.map((resource, index) => {
            // Get icon and styling for the resource
            const styling = getResourceIcon(resource.type)
            const Icon = styling.icon
            const cardBg = styling.cardBg
            const iconBg = styling.iconBg
            const iconColor = styling.iconColor
            const borderColor = styling.borderColor
            const hoverBorder = styling.hoverBorder
            const hoverBg = styling.hoverBg
